...
### Steps:
1. **CLIENT creates parcel** → `/client/parcels/new` → POST `/api/parcels`
2. **AGENT validates & accepts** → `/agent/parcels/:id` → PATCH `/api/parcels/:id/validate`
...
2. Backend → Fapshi → sends prompt to phone
...
2. Login with phone + password → POST `/api/auth/login`

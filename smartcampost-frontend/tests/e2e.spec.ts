...
test.beforeEach(async ({ page }) => {
  ...
  await page.route('**/api/auth/login/otp/request', (route) => route.fulfill({ status: 200 }));
  await page.route('**/api/auth/login/otp/confirm', (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ userId: '1', fullName: 'E2E Tester', accessToken: 'fake-jwt-token', role: 'CLIENT' }) }));
  await page.route('**/api/auth/register', (route) => route.fulfill({ status: 201 }));
  await page.route('**/api/parcels**', (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [], totalPages: 0, totalElements: 0, size: 0, number: 0, first: true, last: true }) }));
});

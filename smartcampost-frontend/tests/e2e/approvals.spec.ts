...
test.describe("Approvals and AI-assisted actions - mocked pages", () => {
  test("Approver can view and approve pending AI requests (mocked isolated page)", async ({
    page,
  }) => {
    ...
    await page.route("**/api/approvals/pending", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify([
          {
            id: "a1",
            toolName: "assignCourierTool",
            actorId: "ai-system",
            parametersJson: "{}",
            reason: "High-value assignment",
            createdAt: Date.now(),
          },
        ]),
      }),
    ...
    // Mock approve action
    await page.route("**/api/approvals/*/approve", (route) =>
    ...
    <html>
      <body>
        ...
        <script>
          (function renderMockedApprovals() {
            const arr = [
              {
                id: 'a1',
                toolName: 'assignCourierTool',
            ...
            document.querySelectorAll('.approve').forEach((btn) =>
              btn.addEventListener('click', async (ev) => {
                ...
                // Call mocked approve endpoint; route handler in test will intercept if used
                try {
                  await fetch('/api/approvals/' + id + '/approve', { method: 'POST' });
                } catch (e) {
                  /* ignore network errors in this isolated DOM */
                }

...
export async function apiCreateUser(
  ...
  user: { email: string; password: string; name?: string; phone?: string },
) {
...
export async function setAuthTokenInLocalStorage(
  page: Page,
  token: string,
  userObj: any = null,
) {
...
export async function ensureLoggedIn(
  ...
  if (!body || !body.accessToken) {
    throw new Error(
      "Login via API did not return accessToken. Adjust test fixture.",
    );
  }
